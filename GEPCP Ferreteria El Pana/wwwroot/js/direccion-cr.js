...
const divisionCR = {
    ...
    "Puntarenas": {
        ...
        "Coto Brus": ["San Vito", "Sabalito", "Aguabuena", "Limoncito", "Pittier", "Gutiérrez Browne"],
        "Parrita": ["Parrita"],
...
function initDireccionCR(provinciaId, cantonId, distritoId, provinciaVal, cantonVal, distritoVal) {
    const selProv = document.getElementById(provinciaId);
    const selCant = document.getElementById(cantonId);
    const selDist = document.getElementById(distritoId);
    if (!selProv || !selCant || !selDist) return;

    // --- POBLAR PROVINCIAS SIEMPRE ---
    selProv.innerHTML = '<option value="">-- Seleccione --</option>';
    Object.keys(divisionCR).forEach(function(p) {
    ...
    function poblarCantones(provincia, seleccionado) {
        selCant.innerHTML = '<option value="">-- Seleccione cantón --</option>';
        selDist.innerHTML = '<option value="">-- Seleccione distrito --</option>';
        if (!provincia || !divisionCR[provincia]) return;
        Object.keys(divisionCR[provincia]).sort().forEach(c => {
            const opt = document.createElement('option');
            opt.value = c;
            opt.textContent = c;
    ...
    function poblarDistritos(provincia, canton, seleccionado) {
        selDist.innerHTML = '<option value="">-- Seleccione distrito --</option>';
        if (!provincia || !canton || !divisionCR[provincia] || !divisionCR[provincia][canton]) return;
        divisionCR[provincia][canton].forEach(d => {
            const opt = document.createElement('option');
            opt.value = d;
            opt.textContent = d;
            if (d === seleccionado) opt.selected = true;
    ...
    function inicializarDireccion(reintentos) {
        ...
        if (provinciaEncontrada) {
            poblarCantones(selProv.value, null);
            setTimeout(function() {
                // Seleccionar cantón
                ...
                if (cantonEncontrado) {
                    poblarDistritos(selProv.value, selCant.value, null);
                    setTimeout(function() {
                        ...
                        if (!distritoEncontrado && reintentos > 0) {
